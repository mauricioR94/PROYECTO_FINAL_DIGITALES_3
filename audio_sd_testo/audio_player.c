...
// =====================================================
// Volumen = muestra >> AUDIO_VOLUME_SHIFT
// 0 -> sin atenuación
// 1 -> /2  (~ -6 dB)
// 2 -> /4  (~ -12 dB)
// 3 -> /8  (~ -18 dB)
// 4 -> /16 (~ -24 dB)
#define AUDIO_VOLUME_SHIFT  3   // PRUEBA así; si sigue distorsionando, sube a 4
...
// Cuántos bytes del chunk data se han leído de la SD (a los buffers)
...
static bool find_data_chunk(FIL *file, uint32_t *data_size, uint32_t *data_position) {
    ...
    while (true) {
        ...
        // Saltar contenido del chunk actual
...
bool audio_player_init() {
    printf("Inicializando reproductor de audio.\n");
...
bool audio_player_play(const char *filename) {
    ...
    if (fr != FR_OK || bytes_read != sizeof(wav_riff_header_t)) {
        printf("✗ Error al leer RIFF header\n");
        f_close(&audio_file);
        file_open    = false;
        player_state = PLAYER_ERROR;
        return false;
    ...
    printf("▶️  Reproduciendo. (%lu bytes)\n", total_bytes);
    return true;
}
...
void audio_player_stop() {
    ...
    printf("⏹️  Reproducción detenida\n");
}
...
void audio_player_process() {
    ...
    if (wav_bits == 16) {
        ...
        // =========================================
        // APLICAR ATENUACIÓN GLOBAL (VOLUMEN)
        // =========================================
